    ...
    (let loop ([index 1]
      ...
      (when (< index 2^m_1)
        (let ([last_val*2^1 #f]
          ...
          (set! last_val*2^1 (poly_a-multiply-n last_val 1))
          (hash-set! poly_index->poly_hash a_index last_val*2^1)

          (printf "    (poly_a-multiply-n ~a 1) = ~a\n" last_val last_val*2^1)
          (hash-set! poly_index->decimal_hash a_index (poly_a->n last_val*2^1))

          (loop (add1 index) last_val*2^1))))
